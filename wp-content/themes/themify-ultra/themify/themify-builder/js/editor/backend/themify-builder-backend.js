(function ($,DOC,api,builderData) {
    ...
    api.render = function () {
        ...
        if ( hasGuttenberg && !canvas) return;
            ...
            if(lock!==null && !lock.classList.contains('hidden')){
                Themify.LoadAsync( builderData.builder_url + '/js/editor/themify-ticks.js', function() {
                        if ( $body[0].classList.contains( 'tb_restriction' ) ) {
                                TB_Ticks.init( builderData.ticks, window ).show();
                        } else {
                                TB_Ticks.init( builderData.ticks, window ).ticks();
                        }
                }, null, null, function() {
				...
				$.ajax({
					...
					success: function ( response ) {
						if ( response != '-1' ) {
							...
							tb_app.Forms.reLoad( data, builderData.post_ID );
						}
					}
				});
			}
