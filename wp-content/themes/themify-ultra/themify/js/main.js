...
(function (window, document,und, $) {
    ...
    Themify = {
        ...
        ajaxSearch(opt){
            opt.type = opt.type && opt.type === 'overlay'?'overlay':'dropdown';
            const ops = opt.type === 'overlay'?'dropdown':'overlay',
                event = opt.type === 'overlay'?'click':'focus';
            if(opt.el){
                ...
                opt.el.addEventListener(event,function(e){
                    e.preventDefault();
                    e.stopImmediatePropagation();
                    if(self.jsLazy['tf-search'] && self.cssLazy['overlay_search_'+opt.type] && extra_css_loaded && !(true===opt.both && !self.cssLazy['overlay_search_'+ops])){
                        Themify.trigger('themify_overlay_search_init',opt);
                        $(opt.el).trigger(event);
                    }else{
                        const _check = function(){
                            if(self.jsLazy['tf-search'] && self.cssLazy['overlay_search_'+opt.type] && extra_css_loaded){
                                Themify.trigger('themify_overlay_search_init',opt);
                                $(opt.el).trigger(event);
                            }
                        };
                        if(!self.cssLazy['overlay_search_'+opt.type] || (true===opt.both && !self.cssLazy['overlay_search_'+ops])){
                            if(true===opt.both && !self.cssLazy['overlay_search_'+ops]){
                                Themify.LoadCss(self.url + '/css/modules/search-form-'+ops+'.css', self.v,null,null,function(){
                                    self.cssLazy['overlay_search_'+ops]=true;
                                    if(!self.cssLazy['overlay_search_'+opt.type]){
                                        Themify.LoadCss(self.url + '/css/modules/search-form-'+opt.type+'.css', self.v,null,null,function(){
                                            self.cssLazy['overlay_search_'+opt.type]=true;
                                            _check();
                                        });
                                    }else{
                                        _check();
                        ...
                        if(!self.jsLazy['tf-search']){
                            Themify.LoadAsync(self.url + '/js/modules/ajax-search.js',function(){
                                self.jsLazy['tf-search']=true;
                                _check();
                            }, self.v);
                        }
                        if(!extra_css_loaded){
                            Themify.LoadCss(opt.css.url, opt.css.v,null,null,function(){
